...
class StateGameMain(GameState):
    def __init__(
        ...
        current_tetromino: dict = None,
        next_tetromino: dict = None,
        frame_counter: int = 0,
    ):
        super().__init__(game)
        if current_tetromino:
            self.current_tetromino = Tetromino(
                current_tetromino["name"],
            ...
            self.current_tetromino = Tetromino(
                random.choice(list(shapes.items()))[0], random.choice(colors)
            )
        ...
        self.score_for_new_shape_placed = (
            100  # Le score à ajouter pour chaque nouvelle forme placée
        )
        self.multiplier_for_new_shape_placed = (
            0.01  # Le multiplieur à multiplier pour chaque nouvelle forme placée.
        )
        self.score_for_line_removed = (
            1000  # Le score à ajouter pour chaque ligne supprimée
        )
        self.multiplier_for_line_removed = (
            0.1  # Le multiplieur à multiplier pour chaque ligne supprimée.
        )
    ...
    def update(self):
        ...
        if self.frame_counter >= self.current_speed:
            ...
            if not self.current_tetromino.try_move(0, 1, self.grid):
                ...
                if (
                    not self.current_tetromino.try_move(0, 0, self.grid)
                    and self.current_tetromino.y == 0
                ):
                    # Game over
                    print("Game Over!")
                ...
                self.multiplier += (
                    self.multiplier * self.multiplier_for_new_shape_placed
                )
                ...
                self.multiplier += (
                    self.multiplier * self.multiplier_for_line_removed * n_lines_removed
                )
